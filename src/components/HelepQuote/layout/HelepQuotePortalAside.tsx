...
import { HelepQuoteRecapCard } from "@/components/HelepQuote/layout/HelepQuoteRecapCard";
...
export function HelepQuotePortalAside() {
  const { flowStep, session } = useHelepQuoteFlowStep();
  const showRecap = flowStep >= HELEP_QUOTE_FLOW_STEP.RECAP && session != null;

  return (
    <div className="space-y-4">
      {showRecap && session ? <HelepQuoteRecapCard quote={session.quote} /> : null}
