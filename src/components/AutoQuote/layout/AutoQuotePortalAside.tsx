...
import { AutoQuoteRecapCard } from "@/components/AutoQuote/layout/AutoQuoteRecapCard";
...
export function AutoQuotePortalAside() {
  const { flowStep, session } = useAutoQuoteFlowStep();
  const showRecap = flowStep >= AUTO_QUOTE_FLOW_STEP.RECAP && session != null;

  return (
    <div className="space-y-4">
      {showRecap && session ? <AutoQuoteRecapCard quote={session.quote} /> : null}
