...
import { getHelepBreakdownTableRows } from "@/lib/helep/helep-breakdown-display";
import type { HelepQuoteResult } from "@/types/helep-insurance";

interface HelepQuoteRecapStepProps {
...
export function HelepQuoteRecapStep({
  quote,
  isSubmitting = false,
  onBack,
  onContinue,
}: HelepQuoteRecapStepProps) {
  const { breakdown, devise } = quote;
  const breakdownTableRows = getHelepBreakdownTableRows(breakdown, devise);
      ...
      <div className="space-y-6 rounded-lg border border-gray-200 bg-white p-4 text-sm text-text-main sm:p-6">
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="space-y-3">
            ...
            <RecapRow label="Formule" value={breakdown.formuleLabel} />
            <RecapRow
