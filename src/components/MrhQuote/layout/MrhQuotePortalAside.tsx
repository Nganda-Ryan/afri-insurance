...
import { MrhQuoteRecapCard } from "@/components/MrhQuote/layout/MrhQuoteRecapCard";
...
export function MrhQuotePortalAside() {
  const { flowStep, session } = useMrhQuoteFlowStep();
  const showRecap = flowStep >= MRH_QUOTE_FLOW_STEP.RECAP && session != null;

  return (
    <div className="space-y-4">
      {showRecap && session ? <MrhQuoteRecapCard quote={session.quote} /> : null}
