...
import { AccidentsQuoteRecapCard } from "@/components/AccidentsQuote/layout/AccidentsQuoteRecapCard";
...
export function AccidentsQuotePortalAside() {
  const { flowStep, session } = useAccidentsQuoteFlowStep();
  const showRecap = flowStep >= ACCIDENTS_QUOTE_FLOW_STEP.RECAP && session != null;

  return (
    <div className="space-y-4">
      {showRecap && session ? <AccidentsQuoteRecapCard quote={session.quote} /> : null}
