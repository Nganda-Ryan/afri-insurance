...
import { EtudesQuoteRecapCard } from "@/components/EtudesQuote/layout/EtudesQuoteRecapCard";
...
export function EtudesQuotePortalAside() {
  const { flowStep, session } = useEtudesQuoteFlowStep();
  const showRecap = flowStep >= ETUDES_QUOTE_FLOW_STEP.RECAP && session != null;

  return (
    <div className="space-y-4">
      {showRecap && session ? <EtudesQuoteRecapCard quote={session.quote} /> : null}
