...
interface HealthQuotePaymentStepProps {
  quote: HealthQuoteResult;
  onBack: () => void;
}

export function HealthQuotePaymentStep({ quote, onBack }: HealthQuotePaymentStepProps) {
  ...
  const handleInitiatePayment = async () => {
    ...
    try {
      toast.info(
        "Le paiement en ligne pour l'assurance santé sera disponible prochainement.",
      );
    } finally {
      ...
      <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <p className="text-sm text-text-main text-opacity-90">
          ...
          <span className="font-bold">
            {formatAutoAmount(quote.breakdown.cotisation_totale, quote.devise)}
          </span>
        </p>
        <p className="text-sm text-text-main text-opacity-90">
