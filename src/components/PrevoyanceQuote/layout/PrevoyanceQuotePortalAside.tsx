...
import { PrevoyanceQuoteRecapCard } from "@/components/PrevoyanceQuote/layout/PrevoyanceQuoteRecapCard";
...
export function PrevoyanceQuotePortalAside() {
  const { flowStep, session } = usePrevoyanceQuoteFlowStep();
  const showRecap = flowStep >= PREVOYANCE_QUOTE_FLOW_STEP.RECAP && session != null;

  return (
    <div className="space-y-4">
      {showRecap && session ? <PrevoyanceQuoteRecapCard quote={session.quote} /> : null}
